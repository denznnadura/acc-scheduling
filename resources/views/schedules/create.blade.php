...
<x-app-layout>
    <x-slot name="pageTitle">Create Schedule</x-slot>
    <x-slot name="breadcrumb">Schedules / Create</x-slot>
    ...
    <style>
        ...
        .form-check:has(.day-checkbox):hover {
            background: #eff6ff !important;
            border-color: var(--acc-primary) !important;
        }

        .form-check:has(.day-checkbox input:checked) {
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="faculty_id" id="faculty_id"
                                ...
                                @foreach ($faculty as $fac)
                                    <option value="{{ $fac->id }}"
                                        {{ old('faculty_id') == $fac->id ? 'selected' : '' }}>
                ...
                <div class="form-section">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                                ...
                                @foreach ($days as $day)
                                    <div class="col-6 col-md-4 col-lg-2">
                                        <div class="form-check"
                                            style="padding: 10px 12px; background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; transition: all 0.2s;">
                                            <input type="checkbox" name="days[]" value="{{ $day }}"
                                                id="day_{{ $day }}" class="form-check-input day-checkbox"
                            ...
                            @enderror
                            <div class="form-help">Select one or more days for this class</div>
                        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-md-6">
                            ...
                            <input type="number" name="max_students" id="max_students"
                                ...
                                value="{{ old('max_students', 40) }}" min="1" required>
                            <div class="form-help">Maximum enrollment capacity</div>
                            @error('max_students')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
        ...
        <script>
            document.getElementById('scheduleForm').addEventListener('change', function(e) {
                if (['start_time', 'end_time', 'faculty_id', 'room_id', 'semester_id'].includes(e.target.id) ||
                    e.target.classList.contains('day-checkbox')) {
                    checkAvailability();
                }
            });
